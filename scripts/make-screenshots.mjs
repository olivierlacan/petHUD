...
async function shot(name, selector, maxH = 99999, pad = 0) {
  await ev(`(function(){var el=document.querySelector(${JSON.stringify(selector)});if(el)el.scrollIntoView({block:"start"});window.scrollBy(0,-8);})()`);
  await wait(180);
...
// 1. life-stage banner (expanded)
await ev(`(function(){var b=document.querySelector(".lifestage.collapsed .ls-head");if(b)b.click();})()`);
await wait(200);
await shot("lifestage", ".lifestage", 440, 6);
await ev(`(function(){var b=document.querySelector(".lifestage:not(.collapsed) .ls-head");if(b)b.click();})()`); // recollapse
await wait(150);

// 2. trends grid (banner collapsed) + your-log weight card
await shot("trends", "#trends", 780, 6);

// 3. analyte detail — cited context + chart with the diet-change marker
...
await wait(250);
await shot("detail", "#detail .overlay-card", 620, 0);
await ev(`document.getElementById("detail-close").click()`); await wait(150);

// 4. conditions — CKD panel + IRIS staging
...
await wait(250);
await shot("conditions", "#conditions", 640, 6);

// 5 & 6. reports — per-report summary + vet-visit card
...
await wait(200);
await shot("vetvisit", ".vet-visit", 360, 0);
await shot("summary", ".report-card", 540, 0);

// 7. owner journal manager
await ev(`document.getElementById("open-journal").click()`);
await waitFor(`document.getElementById("jr-weight-form") !== null`);
await wait(200);
await shot("journal", "#journal .overlay-card", 560, 0);
