...
### When a report won't parse

Failures are always attributed to **one file**. Importing a pile of PDFs where
one of them is bad tells you which one, and why:

- **In the browser**, a “reports that need attention” panel stays on screen
  (a toast would be overwritten by the next file within milliseconds). Each row
  names the file, the cause — password-protected, corrupt, no selectable text
  (a scan), not an IDEXX report, no date, no values read — and offers **Delete
  report** plus **Copy details** for a bug report. A bad file never stops the
  rest of the batch.
- **Stored reports** are checked the same way on every rebuild
  (`web/lib/diagnose.js`). One that the payload builder can't consume is
  quarantined and listed by filename instead of taking the whole corpus down
  with an error that names nothing.
- **If the app still can't render**, boot falls back to a recovery screen
  listing every report in your browser — suspects first — each with a Delete
  button, so the offender can always be isolated and removed. Your PDFs on disk
  are never touched.
- **In the CLI**, `bin/pethud import` prints `! <file> -> FAILED <error>` and
  keeps going, flags thin reports (`? <file>: no values parsed`), and exits
...
web/                    viewer + the in-browser build
  ...
  lib/                  browser pipeline: extract, parser, resolver, aggregate, db, process, diagnose (ES modules)
...
test/import-errors.mjs  per-file failure attribution + aggregate fault isolation (`npm test`)
